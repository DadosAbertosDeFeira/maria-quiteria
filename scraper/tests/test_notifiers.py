...
from ..notifiers.discord import DiscordMessageManager


@pytest.fixture
def logger_info(mocker):
    return mocker.patch("scraper.notifiers.discord.logger.info")


@pytest.fixture
def request_post(mocker):
    return mocker.patch("scraper.notifiers.discord.requests.post")
...
def test_log_error_when_api_return_an_error(mocker, request_post):
    ...
    logger_error = mocker.patch("scraper.notifiers.discord.logger.error")
    error_message = "Failed to send message. Discord api error: %s"
